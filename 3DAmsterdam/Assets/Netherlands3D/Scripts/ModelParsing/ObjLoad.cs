...
public class ObjLoad : MonoBehaviour
{
	...
	private bool ParseObjLine(ref string objline)
	{
		...
		switch (tag)
		{
			...
			case objTag.usemtl:
				...
				break;
			case objTag.O:
				startposition = objline.IndexOf('o') + 1;
				if (SplitNestedObjects) buffer.AddObject(objline.Substring(startposition));
				break;
			case objTag.V:
				int startpos = 1;
				double x = StringManipulation.ParseNextDouble(objline, ' ', startpos, out startpos);
				double y = StringManipulation.ParseNextDouble(objline, ' ', startpos, out startpos);
				double z = StringManipulation.ParseNextDouble(objline, ' ', startpos, out startpos);
				Vector3RD coord = new Vector3RD(x,y,z);
			...
			case objTag.f:
				...
				else
				{
					...
					return true; //Return failure. Not triangulated.
				}
				break;
		}
		return true;
    ...
    {
		if (objLine.IndexOf(MTLLIB) ==0)
		{
			return objTag.mtllib;
		}
		if (objLine.IndexOf(USEMTL) == 0)
		{
			return objTag.usemtl;
		}
		if (objLine.IndexOf(VT) == 0)
		{
			return objTag.vt;
		}
		if (objLine.IndexOf(VN) == 0)
		{
			return objTag.vn;
		}
		if (objLine.IndexOf(V) == 0)
        {
			return objTag.V;
        }
		if (objLine.IndexOf(F) == 0)
		{
			return objTag.f;
		}
		if (objLine.IndexOf(O) == 0)
		{
			return objTag.O;
		}
		return objTag.none;
