...
Shader "Universal Render Pipeline/NiloCat Extension/Screen Space Decal/Unlit"
{
    Properties
    {
        ...
        //_HeatGradientTex("Gradient for data", 2D) = "white" {}
...
#endif
                half inputValue = tex2D(_MainTex, uv).r;

                float normalized = saturate((inputValue - _minValue) / (_maxValue - _minValue));
                half4 col = tex2D(_HeatGradientTex, float2(normalized, 0.5f));
                col.a = 0.3f; // saturate(normalized * 0.3f);

                // Ignore if too small of a value.
                if (normalized <= 0.00001f) discard;
